...
void test_request(void)
{
  ...
  pelz_decrypt_request_handler(eid, &status, request_type, tmp, data_in, &output);
  CU_ASSERT(status == KEK_NOT_LOADED);
  free_charbuf(&tmp);
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    pelz_decrypt_request_handler(eid, &status, request_type, tmp, data, &output);
    CU_ASSERT(status == REQUEST_OK);
  ...
  pelz_encrypt_request_handler(eid, &status, REQ_ENC, tmp, data_in, &output);
  CU_ASSERT(status == KEK_NOT_LOADED);
  free_charbuf(&tmp);
  ...
  pelz_encrypt_request_handler(eid, &status, request_type, tmp, data, &output);
