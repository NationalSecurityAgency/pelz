				       ...
				       size_t* tag_len)
{
  ...
  *cipher = (unsigned char *) malloc(*cipher_len);
  ...
  if (!EVP_EncryptUpdate(ctx, *cipher, &tmp_len, plain, plain_len))
  {
    pelz_log(LOG_ERR, "encryption error ... exiting");
    free(*cipher);
    *cipher = NULL;
    *cipher_len = 0;
    EVP_CIPHER_CTX_free(ctx);
  ...
  if (!EVP_EncryptFinal_ex(ctx, (*cipher) + ciphertext_len, &tmp_len))
  {
				       ...
				       size_t* plain_len)
{
  ...
  *plain = NULL;
  ...
  if(iv != NULL || iv_len > 0 || tag != NULL || tag_len > 0)
  {
    pelz_log(LOG_ERR, "Unexpected non-NULL arguments to decryptor.");
    return 1;
  }
  ...
  *plain = (unsigned char *) malloc(cipher_len);
  ...
  if (!EVP_DecryptUpdate(ctx, *plain, &tmp_len, cipher, cipher_len))
  {
