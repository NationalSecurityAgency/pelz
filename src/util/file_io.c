...
int verifyInputFilePath(char *path)
{
  ...
  {
    pelz_log(LOG_ERR, "input file (%s) not found ... exiting", path);
    return 1;
  }
  ...
  {
    pelz_log(LOG_ERR, "input file (%s) stats could not be retrieved ... exiting", path);
    return 1;
  }
  if (S_ISREG(buffer.st_mode) == 0)
  {
    pelz_log(LOG_ERR, "input file (%s) is not a regular file ... exiting", path);
    return 1;
  }
  ...
  {
    pelz_log(LOG_ERR, "input file (%s) not readable ... exiting", path);
    return 1;
  }
...
int verifyOutputFilePath(char *path)
{
  ...
  if (path == NULL)
  {
    pelz_log(LOG_ERR, "NULL output path ... exiting");
    return 1;
  }
  ...
  {
    pelz_log(LOG_ERR, "unable to copy output file path ... exiting");
    return 1;
  }
  struct stat buffer = { 0 };
  if (stat(dirname(path_copy), &buffer))
  {
    pelz_log(LOG_ERR, "output path (%s) not found ... exiting", path);
    free(path_copy);
    return 1;
  }
  ...
  {
    pelz_log(LOG_ERR, "output directory (%s) not valid ... exiting", dirname(path_copy));
    free(path_copy);
    return 1;
  }
  free(path_copy);
  ...
  if (!stat(path, &buffer))
  {
    if (S_ISREG(buffer.st_mode) == 0)
    {
      pelz_log(LOG_ERR, "output path (%s) is not a regular file ... exiting", path);
      return 1;
    }
  }
  ...
  if (!stat(path, &buffer))
  {
    ...
    {
      pelz_log(LOG_ERR, "output file (%s) not writeable ... exiting", path);
      return 1;
    }
  }
...
int read_bytes_from_file(char *input_path, uint8_t ** data, size_t * data_length)
{
  ...
  {
    pelz_log(LOG_ERR, "unable to create BIO ... exiting");
    return 1;
  }
  ...
  {
    pelz_log(LOG_ERR, "error opening input file: %s ... exiting", input_path);
    if (!BIO_free(bio))
    {
      pelz_log(LOG_ERR, "error freeing BIO ... exiting");
    }
    return 1;
  }
  ...
  {
    pelz_log(LOG_ERR, "input file (%s) stats could not be retrieved ... exiting", input_path);
    if (!BIO_free(bio))
    {
      pelz_log(LOG_ERR, "error freeing BIO ... exiting");
    }
  ...
  *data = (uint8_t *) malloc(input_size);
  if (data == NULL)
  {
    pelz_log(LOG_ERR, "could not allocate memory to read file ... exiting");
    if (!BIO_free(bio))
    {
      pelz_log(LOG_ERR, "error freeing BIO ... exiting");
    }
  ...
  *data_length = BIO_read(bio, *data, input_size);
  if (*data_length <= 0)
  {
    pelz_log(LOG_ERR, "no data read from input file ... exiting");
    if (!BIO_free(bio))
    {
      pelz_log(LOG_ERR, "error freeing BIO ... exiting");
    }
  ...
  {
    pelz_log(LOG_ERR, "file size = %d bytes, buffer size = %d bytes " "... exiting", input_size, *data_length);
    if (!BIO_free(bio))
    {
      pelz_log(LOG_ERR, "error freeing BIO ... exiting");
    }
...
int write_bytes_to_file(char *output_path, uint8_t * bytes, size_t bytes_length)
{
  ...
  {
    pelz_log(LOG_ERR, "invalid output path (%s) ... exiting", output_path);
    return 1;
  }
  ...
  if (file == NULL)
  {
    pelz_log(LOG_ERR, "unable to open file: %s ... exiting", output_path);
    return 1;
  }
  pelz_log(LOG_DEBUG, "opened file \"%s\" for writing", output_path);

  if (fwrite(bytes, sizeof(uint8_t), bytes_length, file) != bytes_length)
  {
    pelz_log(LOG_ERR, "Error writing file ... exiting");
    return 1;
  }
...
int print_to_stdout(unsigned char *data, size_t data_size)
{
  ...
  {
    pelz_log(LOG_ERR, "error creating stdout file BIO ... exiting");
    return 1;
  }
